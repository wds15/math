...
namespace stan {
  namespace math {
    ...
    mdivide_left(const Eigen::Matrix<fvar<T>, R1, C1> &A,
      ...
      Eigen::Matrix<T, R1, C2> inv_A_mult_deriv_b
        = mdivide_left(val_A, deriv_b);
      Eigen::Matrix<T, R1, C1> inv_A_mult_deriv_A
        = mdivide_left(val_A, deriv_A);

      Eigen::Matrix<T, R1, C2> deriv
        = inv_A_mult_deriv_b - multiply(inv_A_mult_deriv_A, inv_A_mult_b);
