...
public:
  ...
  operator()(const T0& t_in,                 // time
             const std::vector<T1>& y_in,    // state
             const std::vector<typename stan::return_type<T1, T2>::type>& f_in,         // dy_dt
             const std::vector<T2>& theta,   // parameters
             const std::vector<double>& x,   // double data
             const std::vector<int>& x_int,  // integer data
             std::ostream* msgs) const {
    ...
    std::vector<typename stan::return_type<T1, T2>::type> fP;
    fP.push_back(f_in.at(1));
    fP.push_back(- theta.at(0) * theta.at(0) * f_in.at(0));
...
public:
  ...
  T operator()(Eigen::Matrix<T, Eigen::Dynamic, 1>& x) const {
    ...
    std::vector<std::vector<T> > ys
      = stan::math::integrate_ode_odeSD(sho, sho_prime, y0, t0, ts, theta, data, data_int, 0);
